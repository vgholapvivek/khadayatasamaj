            ...
            <div class="row mb-2">
                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        @can('job-create')
                            <div>
                                <a href="{{  url('admin/job/create') }}" class="btn btn-primary btn-sm text-white mb-0 me-0"
        ...
        <div class="container-fluid">
            ...
            <div class="row">
                <div class="col-12">
                    ...
                    @foreach($job as $item)
                            <tr>
                                <td>{{ (($job->currentPage() - 1 ) * $job->perPage() ) + $loop->iteration }}</td>
                                <td>@if($item->image)<img src="{{ asset($item->image) }}" style="width:30%" alt="Banner Image">@endif</td>
                                <td>@if($item->mobile_image)<img src="{{ asset($item->mobile_image) }}" style="width:30%" alt="Mobile Banner Image">@endif</td>
                                <td>{{ @$item->sequence }}</td>
                                <td>{{@$item->type}}</td>
                                <td>
                                ...
                                <td>
                                    @can('job-edit')
                                        <a href="{{ url('admin/job/' . $item->id . '/edit') }}" title="Edit City">
